...
def make_move_animation(position,sq1,sq2,piece):
    start_x,start_y = f_utl.middlesquare2pos(sq1)
    end_x,end_y = f_utl.middlesquare2pos(sq2)
    visual_steps = int(f_utl.dist(sq2[0]-sq1[0],sq2[1]-sq1[1])*10)
    for i in range(visual_steps+1):
        actualiserfenetre(position,coord_piece = sq1 ,grabed_piece = piece, mousepos = ((1-i/visual_steps)*start_x+(i/visual_steps)*end_x,(1-i/visual_steps)*start_y+(i/visual_steps)*end_y))
        time.sleep(0.01)